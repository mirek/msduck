...
#[test]
fn interrupt_then_drain_preserves_prior_transaction_work() {
    const CHILD: &str = "MSDUCK_PENDING_DRAIN_PROBE_CHILD";
    if std::env::var_os(CHILD).is_none() {
        ...
        let mut child = std::process::Command::new(std::env::current_exe().unwrap())
            .args([
                "--exact",
                "interrupt_then_drain_preserves_prior_transaction_work",
                "--nocapture",
            ])
            .env(CHILD, "1")
            .spawn()
    ...
    for threads in [1, 4] {
        for ending in ["autocommit", "commit", "rollback"] {
            ...
            unsafe { ffi::duckdb_interrupt(connection.0) };
            drop(pending);
            ...
            // Cleanup drains the interrupted executor before InitialCleanup resets
            // the connection interrupt flag for this new query.
