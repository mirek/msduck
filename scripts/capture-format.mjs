...
// Replicates refuseFixtureOutput from PR #312 (not yet on main): the artifact
// path must never resolve to the retained fixture, including through symlinked
// directories or a file that does not exist yet. A hard link to an existing
// fixture is caught by comparing device and inode.
...
async function refuseFixtureOutput(path, retained) {
  if (await canonicalPath(path) === await canonicalPath(retained) || await sameFile(path, retained)) {
    throw new Error('refusing to write capture output over retained fixture ' + fileURLToPath(retained))
  }
}

// Refuse before any directory is created or container starts.
