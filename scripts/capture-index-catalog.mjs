...
const indexSql = `SELECT s.name AS schema_name,o.name AS table_name,i.name,i.index_id,i.type,i.type_desc,i.is_unique,i.data_space_id,i.ignore_dup_key,i.is_primary_key,i.is_unique_constraint,i.fill_factor,i.is_padded,i.is_disabled,i.is_hypothetical,i.allow_row_locks,i.allow_page_locks,i.has_filter,i.filter_definition,i.auto_created,i.optimize_for_sequential_key,i.is_ignored_in_optimization,i.compression_delay,i.suppress_dup_key_messages FROM sys.indexes i JOIN sys.objects o ON o.object_id=i.object_id JOIN sys.schemas s ON s.schema_id=o.schema_id WHERE o.type='U' AND s.name IN ('dbo','alt') ORDER BY s.name,o.name,i.index_id`
const columnSql = `SELECT s.name AS schema_name,o.name AS table_name,i.name AS index_name,ic.index_id,ic.index_column_id,ic.column_id,c.name AS column_name,ic.key_ordinal,ic.partition_ordinal,ic.is_descending_key,ic.is_included_column,ic.column_store_order_ordinal,ic.data_clustering_ordinal FROM sys.index_columns ic JOIN sys.indexes i ON i.object_id=ic.object_id AND i.index_id=ic.index_id JOIN sys.objects o ON o.object_id=ic.object_id JOIN sys.schemas s ON s.schema_id=o.schema_id LEFT JOIN sys.columns c ON c.object_id=ic.object_id AND c.column_id=ic.column_id WHERE o.type='U' AND s.name IN ('dbo','alt') ORDER BY s.name,o.name,ic.index_id,ic.index_column_id`
const programs = [
