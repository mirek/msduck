...
const output = process.argv[2] ?? 'reference/space-properties.json'
const query = "SELECT SPACE(@n),N'a'+SPACE(@n)+N'b'"
const programs = [
 ['constant SPACE', 'SELECT SPACE(3) AS a,SPACE(0) AS b,SPACE(-1) AS c,SPACE(NULL) AS d'],
 ['column SPACE', 'SELECT SPACE(n) AS s FROM (VALUES(1),(NULL),(-1)) t(n)'],
 ['empty column SPACE', 'SELECT SPACE(n) AS s FROM (VALUES(1)) t(n) WHERE 1=0'],
]
