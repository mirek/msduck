  ...
  return messages.map(hex => {
    ...
    for (let at=0; at<b.length;) {
      ...
      if ([0xfd,0xfe,0xff].includes(token)) {
        ...
        const start=at
        const count=b.readUInt16LE(at+1); at+=3
        assert.equal(count,1,'captured computation has one output column')
        const userType=b.readUInt32LE(at); at+=4
        const flags=b.readUInt16LE(at); at+=2
        const type=b[at++]
        let length=8
        if(type===0x6d) {length=b[at++]; assert.equal(length,8)}
        else assert.equal(type,0x3e,'expected floating computation result')
        const nameLength=b[at++]*2
        assert.ok(at+nameLength<=b.length)
        const name=b.subarray(at,at+nameLength).toString('utf16le'); at+=nameLength
        tokens.push({token,userType,flags,type,length,name,rawHex:b.subarray(start,at).toString('hex')})
      } else {
...
const computation = 'SELECT SUM(CAST(a.object_id AS FLOAT)*b.object_id) AS work FROM sys.all_objects a CROSS JOIN sys.all_objects b CROSS JOIN sys.all_objects c OPTION(MAXDOP 1)'
...
async function oneCase(config, entry) {
  return isolatedReference(config, async connection => {
    ...
    try {
      ...
      const body = `INSERT dbo.cancel_probe VALUES(1); IF @hold=1 ${computation}; INSERT dbo.cancel_probe VALUES(2); SELECT 42 AS completed`
      ...
      let finished = false, packets, cancellation, active, schedulerObservation
      ...
      try {
        ...
        while (true) {
          ...
          const observation = await command(observer, `SELECT r.command, SUBSTRING(t.text,r.statement_start_offset/2+1,(CASE WHEN r.statement_end_offset=-1 THEN DATALENGTH(t.text) ELSE r.statement_end_offset END-r.statement_start_offset)/2+1) AS active_statement, r.wait_type FROM sys.dm_exec_requests r CROSS APPLY sys.dm_exec_sql_text(r.sql_handle) t WHERE r.session_id=${spid}`)
          const row = observation.sets[0].rows.find(row => row[0] === 'SELECT' && row[1] === computation)
          if (row) {active=row.slice(0,2); schedulerObservation=observation; break}
          assert.ok(Date.now() < deadline, 'observer did not establish the active computation SELECT')
          await delay(10)
        }
      ...
      let responses
      try {responses=decodeResponses(responseMessages(packets),transactionDescriptor)}
      catch(error) {error.capture={entry,packets,transactionDescriptor,schedulerObservation};throw error}
      return canonical({entry, setup, active, schedulerObservation, cancellation, events:cancellationEvents, reuse, preparedReuse, serverHandleReuse, responses, rawResponses:responseMessages(packets), transactionDescriptor, packets})
    } finally {observer.close()}
  })
}

if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const output = resolve(process.argv[2] ?? 'artifacts/compatibility/attention-compute-reference')
  ...
  await withReferenceContainer(async (config, container) => {
    ...
    for (let repeat=0; repeat<2; repeat++) {
      ...
      for (const entry of cases) {
        try {results.push(await oneCase(config, entry))}
        catch(error) {
          await writeFile(resolve(output,'failure.json'),JSON.stringify({entry,error:error.message,capture:error.capture??null},null,2)+'\n')
          throw error
        }
    ...
    let fixture
    try {fixture = JSON.parse(await readFile(new URL('../reference/attention-compute.json', import.meta.url),'utf8'))}
