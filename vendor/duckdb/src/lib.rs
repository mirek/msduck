...
impl Connection {
    /// Register a C API scalar definition, retaining ownership in the caller.
    ///
    /// # Safety
    /// The definition must be live and its callbacks must uphold DuckDB's C API
    /// contracts for the lifetime of all connections and plans using it.
    pub unsafe fn register_scalar_function_raw(&self, function: ffi::duckdb_scalar_function) -> Result<()> {
        let connection = self.db.borrow_mut();
        let state = unsafe { ffi::duckdb_register_scalar_function(connection.con, function) };
        if state == ffi::DuckDBSuccess {
            Ok(())
        } else {
            Err(Error::DuckDBFailure(ffi::Error::new(state), Some("could not register scalar function".into())))
        }
    }

    /// Register a C API aggregate function on this connection's database.
    /// The caller retains ownership of the function definition handle.
    ///
    /// # Safety
    /// `function` must be a live DuckDB aggregate function definition. Its
    /// callbacks must uphold the C API contracts and remain valid for the
    /// lifetime of all connections and plans that use the registered function.
    pub unsafe fn register_aggregate_function(&self, function: ffi::duckdb_aggregate_function) -> Result<()> {
        let connection = self.db.borrow_mut();
        let state = unsafe { ffi::duckdb_register_aggregate_function(connection.con, function) };
        if state == ffi::DuckDBSuccess {
            Ok(())
        } else {
            Err(Error::DuckDBFailure(ffi::Error::new(state), Some("could not register aggregate function".into())))
        }
    }
