...
impl Statement<'_> {
    ...
    /// Describe bound result columns without executing the statement.
    ///
    /// Returned names and logical types are owned. Unlike `schema`, this does
    /// not require an executed result and never uses a previous result schema.
    /// Unresolved parameter-dependent result types return an error. These are
    /// preparation declarations, not a schema for decoding executed vectors.
    pub fn prepared_columns(&self) -> Result<Vec<(String, crate::core::LogicalTypeHandle)>> {
        self.stmt.prepared_columns()
    }
