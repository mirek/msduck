...
impl RawStatement {
    ...
    pub fn prepared_columns(&self) -> Result<Vec<(String, LogicalTypeHandle)>> {
        let count = unsafe { ffi::duckdb_prepared_statement_column_count(self.ptr) as usize };
        let mut columns = Vec::with_capacity(count);
        for index in 0..count {
            unsafe {
                let name = ffi::DuckDbString::from_nullable_ptr(
                    ffi::duckdb_prepared_statement_column_name(self.ptr, index as u64),
                ).ok_or_else(|| Error::DuckDBFailure(
                    ffi::Error::new(ffi::DuckDBError),
                    Some(format!("Could not retrieve prepared column name at index {index}")),
                ))?;
                let kind = logical_type_from_duckdb_column(
                    ffi::duckdb_prepared_statement_column_logical_type(self.ptr, index as u64), index,
                )?;
                // The C API can report one placeholder column when binding has
                // not resolved the result types (for example untyped parameters).
                if kind.id() == LogicalTypeId::Invalid {
                    return Err(Error::DuckDBFailure(
                        ffi::Error::new(ffi::DuckDBError),
                        Some(format!("Unresolved prepared column type at index {index}")),
                    ));
                }
                columns.push((name.to_string_lossy().into_owned(), kind));
            }
        }
        Ok(columns)
    }
