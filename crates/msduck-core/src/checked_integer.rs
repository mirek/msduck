...
pub fn calculate(operation: Operation, left: Integer, right: Integer) -> Result<Integer, SqlError> {
    ...
    let (Some(left), Some(right)) = (left.value(), right.value()) else {
        return Ok(if big { Integer::BigInt(None) } else { Integer::Int(None) });
    };
    ...
    let overflow = || SqlError::new(8115, 2, format!("Arithmetic overflow error converting expression to data type {name}."));
    ...
    Ok(if big { Integer::BigInt(Some(value as i64)) } else { Integer::Int(Some(value as i32)) })
}
