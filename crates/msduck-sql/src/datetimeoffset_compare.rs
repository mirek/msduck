//! Compare exact DATETIMEOFFSET values independently of their declared scale.
use sqlparser::ast::*;
...
pub fn scale(
    ...
    match expr {
        ...
        Expr::Function(function) => {
            if let Some(scale) =
                crate::expression_metadata::temporal::datetimeoffsetfromparts_scale(function)
            {
                return Some(scale);
            }
            ...
            if name == "switchoffset"
                && let FunctionArguments::List(args) = &function.args
                && let Some(FunctionArg::Unnamed(FunctionArgExpr::Expr(value))) = args.args.first()
            {
                return Some(scale(value, parameters).unwrap_or(7));
            }
            if name == "todatetimeoffset"
                && let FunctionArguments::List(args) = &function.args
                && let Some(FunctionArg::Unnamed(FunctionArgExpr::Expr(value))) = args.args.first()
            {
                return Some(crate::datetime2_compare::scale(value, parameters).unwrap_or(7));
            }
            let suffix = name
                .strip_prefix("__msduck_datetimeoffset_cast_")
...
pub fn key(value: Expr) -> Expr {
    crate::expr::binary_function(
        "struct_extract",
        crate::datetimeoffset_cast::convert(value, 7),
        Expr::Value(Value::SingleQuotedString("__msduck_datetimeoffset_7".into()).into()),
    )
}
