...
Three more samples split the latter interval: `column_catalog::sync` to
`object_catalog::sync_lob` took 64–66 ms, while `sync_lob` to the next phase
took 7–8 ms. Column synchronization is another candidate after the active
system-column runtime claim releases `src/column_catalog.rs`.
